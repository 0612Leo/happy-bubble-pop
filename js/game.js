...
class GameEngine {
  constructor() {
    ...
    this.selectedMode = MODES.SINGLE;
    ...
    this.playerScores = { left: 0, right: 0 };
    this.level = 1;
    ...
    this.dom = {
      ...
      duoModeBtn: document.getElementById('duoModeBtn'),
      modeHint: document.getElementById('modeHint'),
      pkScoreboard: document.getElementById('pkScoreboard'),
      leftPlayerScore: document.getElementById('leftPlayerScore'),
      rightPlayerScore: document.getElementById('rightPlayerScore'),
      pkTargetScore: document.getElementById('pkTargetScore'),
      ...
      duoResultPanel: document.getElementById('duoResultPanel'),
      finalLeftScore: document.getElementById('finalLeftScore'),
      finalRightScore: document.getElementById('finalRightScore')
    };
  ...
  initEvents() {
    // Mode selector
    [this.dom.singleModeBtn, this.dom.duoModeBtn].forEach((button) => {
      if (!button) return;
      button.addEventListener('click', () => {
        this.setSelectedMode(button.dataset.mode || MODES.SINGLE);
      });
    });
  ...
  setSelectedMode(mode) {
    this.selectedMode = mode === MODES.DUO ? MODES.DUO : MODES.SINGLE;
    const isDuo = this.selectedMode === MODES.DUO;

    if (this.dom.singleModeBtn) this.dom.singleModeBtn.classList.toggle('active', !isDuo);
    if (this.dom.duoModeBtn) this.dom.duoModeBtn.classList.toggle('active', isDuo);

    if (this.dom.modeHint) {
      this.dom.modeHint.textContent = isDuo
        ? `粉队和蓝队抢泡泡，谁先拍到 ${CONFIG.PK_TARGET_SCORE} 个谁获胜。`
        : '一个人自由拍泡泡，完成 5 关挑战。';
    }
  }
  ...
  isDuoMode() {
    return this.selectedMode === MODES.DUO;
  }

  updateModeHUD() {
    const isDuo = this.isDuoMode();

    if (this.dom.pkScoreboard) {
      this.dom.pkScoreboard.classList.toggle('hidden', !isDuo);
    }

    if (this.dom.pkTargetScore) {
      this.dom.pkTargetScore.textContent = CONFIG.PK_TARGET_SCORE;
    }

    if (this.dom.hudHint) {
      this.dom.hudHint.textContent = isDuo
        ? '✨ 粉队用左手，蓝队用右手，抢先拍泡泡！✨'
        : '✨ 挥挥小手，把所有的大泡泡拍碎吧！✨';
    }
  }
  ...
  updatePkScoreUI() {
    if (this.dom.leftPlayerScore) this.dom.leftPlayerScore.textContent = this.playerScores.left;
    if (this.dom.rightPlayerScore) this.dom.rightPlayerScore.textContent = this.playerScores.right;
  }

  updateGameOverUI() {
    const isDuo = this.isDuoMode();
    if (this.dom.singleResultPanel) this.dom.singleResultPanel.classList.toggle('hidden', isDuo);
    if (this.dom.duoResultPanel) this.dom.duoResultPanel.classList.toggle('hidden', !isDuo);

    if (isDuo) {
      const leftScore = this.playerScores.left;
      const rightScore = this.playerScores.right;
      const winner = leftScore === rightScore ? '平局啦！' : leftScore > rightScore ? '粉队胜利！' : '蓝队胜利！';
      if (this.dom.gameOverTitle) this.dom.gameOverTitle.textContent = `🏆 ${winner} 🏆`;
      if (this.dom.gameOverSubtitle) this.dom.gameOverSubtitle.textContent = '泡泡 PK 结束，两个小玩家都很棒！';
      if (this.dom.finalLeftScore) this.dom.finalLeftScore.textContent = leftScore;
      if (this.dom.finalRightScore) this.dom.finalRightScore.textContent = rightScore;
    } else {
      if (this.dom.gameOverTitle) this.dom.gameOverTitle.textContent = '🌟 太棒了！🌟';
      if (this.dom.gameOverSubtitle) this.dom.gameOverSubtitle.textContent = '你完成了超级泡泡挑战！';
  ...
  update(dt) {
    ...
    for (let i = this.items.length - 1; i >= 0; i--) {
      ...
      const getHitDistance = (handData) => {
        if (!handData || !handData.landmarks) return Infinity;
      ...
      const leftHitDistance = getHitDistance(this.hands.left);
      const rightHitDistance = getHitDistance(this.hands.right);

      if (leftHitDistance !== Infinity || rightHitDistance !== Infinity) {
        const scoringHand = leftHitDistance <= rightHitDistance ? 'left' : 'right';
        this.handleItemCatch(item, scoringHand);
  ...
  handleItemCatch(item, scoringHand = null) {
    this.score++;
    this.starsCount++;
    if (this.isDuoMode() && scoringHand && this.playerScores[scoringHand] !== undefined) {
      this.playerScores[scoringHand]++;
      this.updatePkScoreUI();
    }
    ...
    if (this.isDuoMode()) {
      this.handleDuoProgress(scoringHand);
      return;
    }
  ...
  handleDuoProgress(scoringHand) {
    ...
    const leftWon = this.playerScores.left >= CONFIG.PK_TARGET_SCORE;
    const rightWon = this.playerScores.right >= CONFIG.PK_TARGET_SCORE;
    if (leftWon || rightWon) {
      this.gameOverPending = true;
      const winnerText = leftWon && rightWon
        ? '平局啦！'
        : scoringHand === 'left'
          ? '粉队胜利！'
          : '蓝队胜利！';
